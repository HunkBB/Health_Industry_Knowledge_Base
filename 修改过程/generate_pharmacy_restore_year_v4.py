...
W,H=960,540
ROOT=Path('\u0030\u0032-\u8fde\u9501\u836f\u5e97'); OUT=Path('assets/doc_cards_web/pharmacy_batch_20260617_unique_v4_restore_year'); PRE=Path('output/pharmacy_batch_20260617_unique_v4_restore_year'); MAP=Path('assets/doc_cards_web/doc_image_web_map.json')
...
def clean_title(t):
 for token in [f'{LP}2025-2026{RP}','(2025-2026)','_2025-2026','2025-2026',f'{LP}{RP}','()',]:
  t=t.replace(token,'')
 t=re.sub(r'\s+',' ',t).strip(' ·-_()'+LP+RP)
 return C['chain'] if t=='_index' or not t else t
def display_title(t):
    raw=t
    if C['chain'] in raw and C['official'] in raw and C['finance'] in raw and C['core'] in raw:
        return '__ONE_LINE__' + C['chain'] + '2025' + C['official'] + C['finance']
    gy=C['gyyz']; gd=C['gd']; news=C['news']; dynamic=C['dynamic']; biz=C['biz']; summary=C['summary']
        # 所有“2025年报与2026Q1经营摘要” -> “企业名年报与摘要”
    if ('2026Q1' in raw or '2025' in raw) and summary in raw:
        base = raw.replace('2026Q1', '').replace('2025', '').replace(biz + summary, '').replace(summary, '')
        for token in [LP, RP, '(', ')', '年报与', '年报', '与', ' ', '·', '-', '_']:
            base = base.replace(token, '')
        if not base:
            base = C['chain']
        return '__ONE_LINE__' + base + '年报与摘要'
# 国药一致/国大药房 · 新闻动态（2025-2026） -> 国药一致/国大药房
    if gy in raw and gd in raw and (news in raw or dynamic in raw):
        return gy + '/' + gd
    # 国药一致／国大药房2025年报与2026Q1经营摘要 -> 国药一致／国大药房年报与摘要
    if gy in raw and gd in raw and ('2026Q1' in raw or '2025' in raw) and summary in raw:
        return '__ONE_LINE__' + gy + '／' + gd + '年报与摘要'
    return clean_title(raw)
def split_title(t):
 t=display_title(t)
 if t.startswith('__ONE_LINE__'):
  return [t.replace('__ONE_LINE__','',1)]
 for sep in [' · ','／','/',':','：']:
...
def draw(title,rel,idx):
 ...
 if len(lines)==1:
  size=50 if len(lines[0])<=12 else 43 if len(lines[0])<=18 else 34 if len(lines[0])<=22 else 30; d.text((100,88),lines[0],font=FB(size),fill=teal)
 else:
 ...
 if '2025' in title or '2026' in title: d.text((724,106),'2025-2026',font=FB(22),fill='#6a8980')
...
if MAP.exists():
 b=MAP.with_name('doc_image_web_map.before-pharmacy-restore-year-v4-20260617.json')
